...
export const scrapperTwitter = async (scrapNew = true) => {
	...
	const isLogined = await page.evaluate(
		() =>
			Array.from(document.querySelectorAll('a')).find((elem) => elem.textContent === 'Log in') ===
			undefined
	);
	if (!isLogined) {
	...
	while (countScrollStuck <= 2 && countNothingNew <= 2) {
		...
		const payload = await page.evaluate(
			async (dataCache, scrollHeight, countScrollStuck) => {
				let data = [];
				const elems =
					document.querySelector('div[aria-label^="Timeline"] > div')?.querySelectorAll('a') ?? [];
				for (let i = 0; i < elems.length; i++) {
					const elem = elems[i];
					const basicUrl = (elem.getAttribute('href') ?? '/').split('/');
					const url =
						basicUrl.length >= 3
							? `https://x.com/${basicUrl[1]}/${basicUrl[2]}/${basicUrl[3]}`
							: null;
