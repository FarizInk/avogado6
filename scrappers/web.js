...
export const scrapperWeb = async () => {
	...
	for (let i = diaryYears.length - 1; i >= 0; i--) {
		...
		const payload = await page.evaluate((dataCache) => {
			let months = [];
			document
				.querySelectorAll('div[data-testid="richTextElement"]')
				.forEach((elem) =>
					months.push(elem.textContent?.replace('.', '-')?.replace('～', '')?.replace('​', ''))
				);
			...
			document.querySelectorAll('div.wixui-gallery').forEach((section) => {
				...
				galeries.forEach((elem) => {
					...
					const uri =
						JSON.parse(elem.querySelector('wow-image').getAttribute('data-image-info'))?.imageData
							.uri ?? null;
					...
					if (!isExist) {
						...
						data.push({
							...
							date,
							identifier:
								sameIdentifier.length >= 1 ? `${identifier}(${sameIdentifier.length})` : identifier,
							id: null
						});
					}
				});
		...
		const filteredData = data.filter((item) =>
			item.date.includes(year ?? new Date().getFullYear())
		);
